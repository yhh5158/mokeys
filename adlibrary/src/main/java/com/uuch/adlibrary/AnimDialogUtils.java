...
public class AnimDialogUtils {
    ...
    public AnimDialogUtils initView(final View customView) {
        ...
        animBackView = (View) rootView.findViewById(R.id.anim_back_view);
    ...
    private OnLoginClickListenner mOnLoginClick;
    /**
     * login单击事件
     */
    public interface OnLoginClickListenner {

        public void goLogin();
    }

    public void setOnLoginClickLinstener(OnLoginClickListenner click){
        this.mOnLoginClick = click;
    }
     ...
     * @return
     */
    public AnimDialogUtils initView(final View customView,boolean login) {
        if (isOverScreen) {
            androidContentView = (ViewGroup) context.getWindow().getDecorView();
        } else {
            androidContentView = (ViewGroup) context.getWindow().findViewById(Window.ID_ANDROID_CONTENT);
        }
        rootView = LayoutInflater.from(context).inflate(R.layout.anim_dialog_layout_mobai, null);
        rootView.setTag(ANIM_DIALOG_TAG);
        ...
        flContentContainer.addView(customView, contentParams);
        ivClose = (ImageView) rootView.findViewById(R.id.iv_close);
        mLoginLin = (LinearLayout)rootView.findViewById(R.id.login_lin);
        if(login){
            mLoginLin.setVisibility(View.VISIBLE);
            mLoginLin.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    mOnLoginClick.goLogin();
                }
            });
        }else{
            mLoginLin.setVisibility(View.GONE);
        }
        return this;
    }
